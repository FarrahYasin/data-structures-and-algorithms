...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    const addTwoNum = arr[i] + 2;
    newArr.push(addTwoNum);
  }
  return newArr;
};
...
const typeNum = (arr) => {
  // Solution code here...
  const newArr=arr.filter((element) => typeof element === 'number');
  return newArr;
};
...
const containsAnd = (arr) => {
  // Solution code here...
  const newArr= arr.filter(element=> typeof element !=='number' && element.includes('and'));
  return newArr;
};
