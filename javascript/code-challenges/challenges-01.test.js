...
const addOne = (arr) => {
  // Solution code here...
  const arr2 = []; //create another array (empty arr)
  arr.forEach(currentValue => { //use  forEach to loop through arr elements (original arr that we passed to addOne function)
    const valAfterIncremente = currentValue +1; //add 1 to each element inside the original array and store it inside new variable
    arr2.push(valAfterIncremente); //push the new variable to the new arr
  });
  return arr2; //return new arr(that we pushed the edit values to it)
};
...
const addExclamation = (arr) => {
  ...
  arr.forEach(currentElement=>{
    const valAfterAdd=currentElement+'!';
    arr2.push(valAfterAdd);
  });
  return arr2;
};
...
const allUpperCase = (arr) => {
  ...
  arr.forEach(currentElem=>{
    const valueAfterConvert=currentElem.toUpperCase();
    arr2.push(valueAfterConvert);
  });
  return arr2;
};
...
const greeting = (word) => {//word its a string
  // Solution code here...
  const newWord=word.toUpperCase() +'!';
  return newWord;
};

const speaker = (words, callback) => {//words its an array
  // Solution code here...
  const words2=[];
  words.forEach((string)=>{
    const newVal=callback(string);//the callback function take string as argument
    words2.push(newVal);
  });
  return words2;
...
const addNumbers = (num, arr, times, callback) => {
  ...
  for (let i = 0; i < times; i++){
    callback(arr, num); // this addValues function
  }
  return arr;
};
...
const createList = (availableItems) => {
  // Solution code here...
  const groceryList = [];
  availableItems.forEach((currentElem) => {
    if (currentElem.available) {
      groceryList.push(currentElem.name);
    }
  });
  return groceryList;
