...
const toLastNames = people => {
  // Solution code here...
  const fullName = people.map((index) => {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const count = arr.reduce((previousVal, currentVal, index) => {
...
const returnNames = (arr) => {
  ...
  arr.reduce((previousVal, currentVal) => {
    newNameArr.push(currentVal.name);
  });
  newNameArr.unshift(starWarsData[0].name);
  return newNameArr;
};
...
const reversedString = (str) => {
  // Solution code here...
  const result = str.split('').reduce((reverse, letters) => letters + reverse, "");
  return result;
};
